...
`flowerpot-web` 是 YSplanter/花盆产品体系的 PC 管理后台前端，负责用户、产品与 FAQ、植物管理、APP 版本与应用市场，以及员工、角色、菜单和部门。2026-09-08 按产品要求下线了用户设备（用户产品列表）、用户产品图片、产品版本和系统配置四个模块（页面、路由与只服务于它们的接口封装一并删除；后端接口与已存在的菜单行不受影响，需要时在后台手工清理菜单）。仓库由旧管理后台迁移而来，`package.json` 名称和 README 仍有 `web-ui-v2` 复制痕迹；项目定位以当前路由、API 和本文件为准。
